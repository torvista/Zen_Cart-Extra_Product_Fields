 ...
 * handles products_description2, code modifications abbreviated by npf
 * @version admin\includes\modules\product\preview_info.php 23 Jan 2026 torvista
 * @var zcObserverPluginExtraProductFields $zcObserverPluginExtraProductFields
 *
 * @copyright Copyright 2003-2025 Zen Cart Development Team
 * @copyright Portions Copyright 2003 osCommerce
 * @license http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
 * @version $Id: DrByte 2025 Sep 17 Modified in v2.2.0 $
 */
...
// bof npf
if (empty($products_description2)) $products_description2 = [];
//eof
...
//eof
  ...
  if (DATE_FORMAT_DATE_PICKER !== 'yy-mm-dd' && !empty($pInfo->products_date_available)) {
    ...
    //bof npf
    //pd.products_description2 added to select
  $product = $db->Execute("SELECT p.*,
        pd.products_description2, pd.language_id, pd.products_name, pd.products_description, pd.products_url
...
//eof
  ...
  foreach($product as $prod) {
    ...
    // bof npf
    $products_description2[$prod['language_id']] = $prod['products_description2'];
    //eof
...
<div class="container-fluid">
    ...
    for ($i = 0, $n = count($languages); $i < $n; $i++) {
      if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
        ...
        //bof npf
        $pInfo->products_description2 = $zcObserverPluginExtraProductFields->npf_get_products_description2($pInfo->products_id, $languages[$i]['id']);
        //eof
...
//auto replace with defined missing image
        ...
        //bof npf
        echo "<h3>products_description</h3>\n" . str_replace('src="images/', 'src="' . DIR_WS_CATALOG_IMAGES, $pInfo->products_description);
        echo "<h3>products_description2</h3>\n" . str_replace('src="images/', 'src="' . DIR_WS_CATALOG_IMAGES, $pInfo->products_description2);
        //eof
  ...
  if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
    ...
    <div class="row text-right">
        ...
        for ($i = 0, $n = count($languages); $i < $n; $i++) {
          ...
          //bof npf
            echo zen_draw_hidden_field('products_description2[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($products_description2[$languages[$i]['id']]), ENT_COMPAT, CHARSET));
            //eof
